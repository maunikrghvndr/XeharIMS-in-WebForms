...
{
    public partial class BodyMentorView : System.Web.UI.Page
    {
        static string SCID = string.Empty;
        ...
        private void BindData()
        {
            ...
            {
                using (MySqlCommand cmd = new MySqlCommand("SELECT p.pid,p.SKU,p.Description,p.ModelNumber,p.Quantity,p.VendorCost, p.ProductName FROM  products as p, productsaleschannels as s where p.pid = s.pid and s.scid=" + GetSCID()))
                {
  ...
  //-------------uncomment to show image----------------------------------------------------------------------------
        ...
        {
            if (!String.IsNullOrEmpty(Request.QueryString["SCID"]))
            {
